<template>
  ...
  <div class="home">
    ...
    <div
      ...
      bg-[url('https://img1.baidu.com/it/u=587373665,2744773399&fm=253&fmt=auto&app=138&f=JPEG?w=889&h=500')] 
      bg-cover bg-center"
    ></div>
    <!-- 搜索+头部 -->
    ...
    <main class="flex flex-col space-y-4">
      ...
      <section>
        <div class="flex justify-between items-center px-6 text-sm text-white">
          <h2 class="title font-bold text-lg">热门贴吧</h2>
          <span
        ...
        <PopularTieba :items="PopularTie" />
      </section>
      <!-- 好贴分享 -->
      <section>
        <div class="flex justify-between items-center px-6 text-sm">
          <h2 class="title font-bold text-lg font-mono">可能喜欢</h2>
        </div>
        <MaybeLike :items="Maylike" />
      </section>
    </main>
  </div>
</template>
<script lang="ts" setup>
import PopularTieba from "@/components/Home/PopularTieba.vue";
import MaybeLike from "@/components/Home/MaybeLike.vue";
//数据？
// pinia 数据管理 + 组件显式
import { ref } from "vue";
...
const { topBarState, navBarState, PopularTie, Maylike } = toRefs(homeStore);
