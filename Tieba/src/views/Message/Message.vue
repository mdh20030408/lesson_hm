              ...
              <div v-for="item in items.items"  class="item" :class="item.type">
                <div v-if="showRec(item.type)" class="text my-2 py-1 flex bg-gray-100 rounded-lg " >
                  ...
                  <div class="ml-1 mt-2 ">
                    ...
                    <div class=" text-left font-semibold mt-4">{{ item.price }}浏览</div>
                    <div class="text-ls text-left text-gray-500 ">获得{{ item.market }}赞</div>
                  </div>
                  <van-icon name="like-o" color="#1989fa" class="relative bottom-[-6.5rem] right-[-1rem]" size="1.5rem" />
                </div>
...
const { topBarState, recommendItemState } = toRefs(messageStore);
